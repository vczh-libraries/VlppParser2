...
- Check `prefix_merge` errors
  - A syntax uses `prefix_merge` should not define rules or placeholders that has `_LRI` or `LRI_`.
  - If a rule has `prefix_merge`, then all other clauses should one in:
    - left recursion clause.
    - normal clause begins with a rule that directly or indirectly begins with `prefix_merge`.
  - A rule cannot directly or indirectly begins with `prefix_merge`, `left_recursion_placeholder` and `left_recursion_inject*`.
