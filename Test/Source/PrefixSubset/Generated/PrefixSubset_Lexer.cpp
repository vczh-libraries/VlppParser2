...
{
	bool PrefixSubsetTokenDeleter(vl::vint token)
	{
		switch((PrefixSubsetTokens)token)
		{
		case PrefixSubsetTokens::SPACE:
			return true;
		default:
			return false;
		}
	...
	const wchar_t* PrefixSubsetTokenId(PrefixSubsetTokens token)
	{
		...
		return 0 <= index && index < PrefixSubsetTokenCount ? results[index] : nullptr;
	}

	const wchar_t* PrefixSubsetTokenDisplayText(PrefixSubsetTokens token)
	{
		static const wchar_t* results[] = {
			L"(",
			L")",
	...
	const wchar_t* PrefixSubsetTokenRegex(PrefixSubsetTokens token)
	{
		static const wchar_t* results[] = {
			L"/(",
			L"/)",
	...
	void PrefixSubsetLexerData(vl::stream::IStream& outputStream)
	{
