/***********************************************************************
This file is generated by: Vczh Parser Generator
From parser definition:PrefixSubset5
Licensed under https://github.com/vczh-libraries/License
***********************************************************************/

#include "PrefixSubset5_Lexer.h"

namespace prefixsubset5
{
	bool PrefixSubset5TokenDeleter(vl::vint token)
	{
		switch((PrefixSubset5Tokens)token)
		{
		case PrefixSubset5Tokens::SPACE:
			return true;
		default:
			return false;
		}
	}

	const wchar_t* PrefixSubset5TokenId(PrefixSubset5Tokens token)
	{
		static const wchar_t* results[] = {
			L"OPEN_ROUND",
			L"CLOSE_ROUND",
			L"OPEN_BRACE",
			L"CLOSE_BRACE",
			L"COMMA",
			L"DOT",
			L"CONST",
			L"ASTERISK",
			L"ID",
			L"SPACE",
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < PrefixSubset5TokenCount ? results[index] : nullptr;
	}

	const wchar_t* PrefixSubset5TokenDisplayText(PrefixSubset5Tokens token)
	{
		static const wchar_t* results[] = {
			L"(",
			L")",
			L"{",
			L"}",
			L",",
			L".",
			L"const",
			L"*",
			nullptr,
			nullptr,
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < PrefixSubset5TokenCount ? results[index] : nullptr;
	}

	const wchar_t* PrefixSubset5TokenRegex(PrefixSubset5Tokens token)
	{
		static const wchar_t* results[] = {
			L"/(",
			L"/)",
			L"/{",
			L"/}",
			L",",
			L".",
			L"const",
			L"/*",
			L"[a-zA-Z_]/w*",
			L"/s+",
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < PrefixSubset5TokenCount ? results[index] : nullptr;
	}

	void PrefixSubset5LexerData(vl::stream::IStream& outputStream)
	{
		static const vl::vint dataLength = 306; // 1907 bytes before compressing
		static const vl::vint dataBlock = 256;
		static const vl::vint dataRemain = 50;
		static const vl::vint dataSolidRows = 1;
		static const vl::vint dataRows = 2;
		static const char* compressed[] = {
			"",
			"\xE8\xBF\xE8\xE7\xEF\xC9\x81\x90\x0D\xEC\xEC\xB2\xEF\xF0\xE8\xE7\xFF\xBE\x7F\x79\x00\x81\x82\xF0\xD8\x9A\xA4\x83\xB9\x81\xBB\x80\x5F\x81\x9D\xA1\x8B\xB2\xFE\xB7\xFD\x00\x00\x88\x58\x80\x00\xA1\x51\x40",
		};
		vl::glr::DecompressSerializedData(compressed, true, dataSolidRows, dataRows, dataBlock, dataRemain, outputStream);
	}
}
