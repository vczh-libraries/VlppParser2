/***********************************************************************
This file is generated by: Vczh Parser Generator
From parser definition:IfElseManual
Licensed under https://github.com/vczh-libraries/License
***********************************************************************/

#include "IfElseManual_Lexer.h"

namespace ifelsemanual
{
	bool IfElseManualTokenDeleter(vl::vint token)
	{
		switch((IfElseManualTokens)token)
		{
		case IfElseManualTokens::SPACE:
			return true;
		default:
			return false;
		}
	}

	const wchar_t* IfElseManualTokenId(IfElseManualTokens token)
	{
		static const wchar_t* results[] = {
			L"OPEN_BRACE",
			L"CLOSE_BRACE",
			L"SEMICOLON",
			L"IF",
			L"CONDITION",
			L"THEN",
			L"ELSE",
			L"DO",
			L"SPACE",
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < IfElseManualTokenCount ? results[index] : nullptr;
	}

	const wchar_t* IfElseManualTokenDisplayText(IfElseManualTokens token)
	{
		static const wchar_t* results[] = {
			L"{",
			L"}",
			L";",
			L"if",
			L"cond",
			L"then",
			L"else",
			L"do",
			nullptr,
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < IfElseManualTokenCount ? results[index] : nullptr;
	}

	const wchar_t* IfElseManualTokenRegex(IfElseManualTokens token)
	{
		static const wchar_t* results[] = {
			L"/{",
			L"/}",
			L";",
			L"if",
			L"cond",
			L"then",
			L"else",
			L"do",
			L"/s+",
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < IfElseManualTokenCount ? results[index] : nullptr;
	}

	void IfElseManualLexerData(vl::stream::IStream& outputStream)
	{
		static const vl::vint dataLength = 251; // 1927 bytes before compressing
		static const vl::vint dataBlock = 256;
		static const vl::vint dataRemain = 251;
		static const vl::vint dataSolidRows = 0;
		static const vl::vint dataRows = 1;
		static const char* compressed[] = {
			"",
		};
		vl::glr::DecompressSerializedData(compressed, true, dataSolidRows, dataRows, dataBlock, dataRemain, outputStream);
	}
}
