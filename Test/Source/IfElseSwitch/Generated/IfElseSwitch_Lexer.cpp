/***********************************************************************
This file is generated by: Vczh Parser Generator
From parser definition:IfElseSwitch
Licensed under https://github.com/vczh-libraries/License
***********************************************************************/

#include "IfElseSwitch_Lexer.h"

namespace ifelseswitch
{
	bool IfElseSwitchTokenDeleter(vl::vint token)
	{
		switch((IfElseSwitchTokens)token)
		{
		case IfElseSwitchTokens::SPACE:
			return true;
		default:
			return false;
		}
	}

	const wchar_t* IfElseSwitchTokenId(IfElseSwitchTokens token)
	{
		static const wchar_t* results[] = {
			L"OPEN_BRACE",
			L"CLOSE_BRACE",
			L"SEMICOLON",
			L"IF",
			L"CONDITION",
			L"THEN",
			L"ELSE",
			L"DO",
			L"SPACE",
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < IfElseSwitchTokenCount ? results[index] : nullptr;
	}

	const wchar_t* IfElseSwitchTokenDisplayText(IfElseSwitchTokens token)
	{
		static const wchar_t* results[] = {
			L"{",
			L"}",
			L";",
			L"if",
			L"cond",
			L"then",
			L"else",
			L"do",
			nullptr,
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < IfElseSwitchTokenCount ? results[index] : nullptr;
	}

	const wchar_t* IfElseSwitchTokenRegex(IfElseSwitchTokens token)
	{
		static const wchar_t* results[] = {
			L"/{",
			L"/}",
			L";",
			L"if",
			L"cond",
			L"then",
			L"else",
			L"do",
			L"/s+",
		};
		vl::vint index = (vl::vint)token;
		return 0 <= index && index < IfElseSwitchTokenCount ? results[index] : nullptr;
	}

	void IfElseSwitchLexerData(vl::stream::IStream& outputStream)
	{
		static const vl::vint dataLength = 251; // 1927 bytes before compressing
		static const vl::vint dataBlock = 256;
		static const vl::vint dataRemain = 251;
		static const vl::vint dataSolidRows = 0;
		static const vl::vint dataRows = 1;
		static const char* compressed[] = {
			"",
		};
		vl::glr::DecompressSerializedData(compressed, true, dataSolidRows, dataRows, dataBlock, dataRemain, outputStream);
	}
}
