...
{
	bool IfElseAmbiguity2TokenDeleter(vl::vint token)
	{
		switch((IfElseAmbiguity2Tokens)token)
		{
		case IfElseAmbiguity2Tokens::SPACE:
			return true;
		default:
			return false;
		}
	...
	const wchar_t* IfElseAmbiguity2TokenId(IfElseAmbiguity2Tokens token)
	{
		...
		return 0 <= index && index < IfElseAmbiguity2TokenCount ? results[index] : nullptr;
	}

	const wchar_t* IfElseAmbiguity2TokenDisplayText(IfElseAmbiguity2Tokens token)
	{
		static const wchar_t* results[] = {
			L"{",
			L"}",
	...
	const wchar_t* IfElseAmbiguity2TokenRegex(IfElseAmbiguity2Tokens token)
	{
		static const wchar_t* results[] = {
			L"/{",
			L"/}",
	...
	void IfElseAmbiguity2LexerData(vl::stream::IStream& outputStream)
	{
