...
namespace ifelsepriority
{
	...
	template<> struct ModuleParserStateTypes<ModuleParserStates::Module> { using Type = ifelsepriority::Module; };

	const wchar_t* ModuleParserRuleName(vl::vint index);
	const wchar_t* ModuleParserStateLabel(vl::vint index);
	extern void IfElsePriorityModuleParserData(vl::stream::IStream& outputStream);

	class ModuleParser: public vl::glr::ParserBase<IfElsePriorityTokens, ModuleParserStates, IfElsePriorityAstInsReceiver, ModuleParserStateTypes>
	{
	public:
		ModuleParser();

		vl::Ptr<ifelsepriority::Module> ParseModule(const vl::WString & input, vl::vint codeIndex = -1);
	};
}
#endif