...
export 1 + 2
		...
		CalculatorAstInsReceiver receiver;
		{
			...
			receiver.Execute({ AstInsType::StackBegin }, tokens[3], 3);
			receiver.Execute({ AstInsType::Token, -1, 0 }, tokens[3], 3);
			receiver.Execute({ AstInsType::CreateObject, (vint32_t)CalculatorClasses::NumExpr }, tokens[3], 3);
			receiver.Execute({ AstInsType::Field, (vint32_t)CalculatorFields::NumExpr_value, 0 }, tokens[3], 3);
			receiver.Execute({ AstInsType::StackEnd }, tokens[3], 3);
			receiver.Execute({ AstInsType::StackSlot, -1, 1 }, tokens[3], 3);
			receiver.Execute({ AstInsType::StackEnd }, tokens[3], 3);
			receiver.Execute({ AstInsType::Token, -1, 2 }, tokens[2], 2);
