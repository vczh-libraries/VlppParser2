...
For grammar maintenance, begin with [BuiltIn-Cpp Syntax Design](SyntaxDesign.md). For the parser architecture behind it, begin with the main [VlppParser2 design guide](../Index.md).
...
- [C++26 Parsing with VlppParser2](Cpp.md) evaluates the parser architecture, frontend boundary, ambiguity model, limitations, and a scalable C++26 pipeline.
...
- To change the BuiltIn-Cpp grammar, read [Syntax Design](SyntaxDesign.md), then the relevant versioned gap file.
...
- To add a post-parse resolver, read [De-ambiguation Improvements](Cases_Improvement.md) and the ambiguity sections in both design documents.
...
BuiltIn-Cpp targets compiler-verified input for a code indexer. It should accept every historically valid standard spelling, preserve choices that require name or type lookup, and may deliberately accept harmless invalid spellings when doing so cannot misinterpret valid input.
